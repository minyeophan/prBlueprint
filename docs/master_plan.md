...
> 위의 버전 선택 가이드가 **"무엇을"** 쓰는지를 알려준다면,
> 이 섹션은 **"왜"** 그것을 쓰는지를 설명합니다.
> 기술 선택에 정답은 없지만, 선택의 이유를 아는 것이 중요합니다.

#### 1) 왜 Java인가? (vs Python, JavaScript/Node.js)

| 비교 항목 | Java | Python | Node.js |
|-----------|------|--------|---------|
| 타입 시스템 | 정적 타입 (컴파일 시 에러 발견) | 동적 타입 | 동적 타입 (TS로 보완 가능) |
| 웹 프레임워크 | Spring Boot | Django, FastAPI | Express, NestJS |
| 학습 곡선 | 높음 (장황한 문법) | 낮음 | 중간 |
| 국내 취업 시장 | 🟢 매우 높음 | 🟡 AI/데이터 중심 | 🟡 스타트업 중심 |
| 대규모 서비스 사례 | 쿠팡, 배민, 토스 | 인스타그램, 드롭박스 | Netflix, PayPal |

> **한 줄 요약**: 정적 타입으로 실수를 컴파일 시점에 잡아주고, Spring 생태계와 국내 취업 수요가 높아 선택했다.
>
> **솔직한 단점**: 코드가 장황하고 초기 학습 곡선이 높다. 하지만 그 장황함이 오히려 "무슨 일이 일어나는지" 명시적으로 보여준다.

#### 2) 왜 Spring Boot인가? (vs Express.js, Django, FastAPI)

| 비교 항목 | Spring Boot | Express.js | Django | FastAPI |
|-----------|-------------|------------|--------|---------|
| 언어 | Java | JavaScript | Python | Python |
| 자동 설정 | ✅ Auto Configuration | ❌ 직접 설정 | ✅ 기본 제공 | ❌ 최소한 |
| 내장 서버 | ✅ Tomcat 내장 | ✅ Node 내장 | ❌ 별도 필요 (Gunicorn 등) | ❌ Uvicorn 필요 |
| 학습 자료 | 🟢 한국어/영어 풍부 | 🟢 영어 풍부 | 🟢 영어 풍부 | 🟡 성장 중 |
| 기업 채용 | 🟢 국내 대기업 표준 | 🟡 스타트업 | 🟡 해외 중심 | 🟡 신규 프로젝트 |

> **한 줄 요약**: Java 웹 프레임워크의 사실상 표준이며, 자동 설정과 내장 서버 덕에 초보자도 빠르게 시작할 수 있다.

#### 3) 왜 PostgreSQL인가? (vs MySQL, MongoDB)

| 비교 항목 | PostgreSQL | MySQL | MongoDB |
|-----------|------------|-------|---------|
| 유형 | 관계형 (RDBMS) | 관계형 (RDBMS) | 문서형 (NoSQL) |
| SQL 표준 준수 | 🟢 매우 높음 | 🟡 일부 비표준 문법 | ❌ 별도 쿼리 언어 |
| JSON 지원 | ✅ jsonb 타입 (인덱싱 가능) | 🟡 기본 JSON 타입 | ✅ 네이티브 |
| 트랜잭션 | ✅ 완전한 ACID | ✅ ACID (InnoDB) | 🟡 4.0부터 지원 |
| 동시 처리 | MVCC (잠금 최소화) | 잠금 기반 | 문서 수준 잠금 |
| 라이선스 | 완전 오픈소스 | GPL (Oracle 소유) | SSPL |

> **한 줄 요약**: 표준 SQL 준수도가 높고, JSON 지원으로 유연하며, 완전한 오픈소스라 선택했다.
>
> **MySQL과의 가장 큰 차이**: MySQL은 더 널리 쓰이지만, PostgreSQL은 고급 기능(Window Function, CTE 등)과 표준 SQL 지원이 뛰어나다.
>
> **MongoDB와의 가장 큰 차이**: 관계형 DB는 데이터 정합성을 보장하고, 스키마가 있어 초보자가 데이터 구조를 이해하기 쉽다.

#### 4) 왜 H2인가? (개발/테스트용)

| 비교 항목 | H2 (개발용) | PostgreSQL (운영용) |
|-----------|------------|-------------------|
| 설치 | ❌ 불필요 (내장) | Docker 또는 직접 설치 |
| 데이터 유지 | 앱 종료 시 삭제 | 영구 저장 |
| 용도 | 빠른 개발, 테스트 | 실제 운영 |
| 전환 비용 | JPA가 SQL 차이를 추상화 → 설정만 변경하면 됨 |

> **한 줄 요약**: 설치 없이 즉시 사용 가능하고, 앱 종료 시 자동 삭제되어 항상 깨끗한 테스트 환경을 제공한다.
>
> **핵심 포인트**: JPA(Java Persistence API)가 SQL 방언 차이를 추상화해주기 때문에, 개발은 H2로 하고 운영은 PostgreSQL로 바꿔도 코드 변경이 거의 없다.

#### 5) 왜 Gradle인가? (vs Maven)

| 비교 항목 | Gradle | Maven |
|-----------|--------|-------|
| 설정 파일 | `build.gradle` (Groovy/Kotlin DSL) | `pom.xml` (XML) |
| 가독성 | 🟢 스크립트 형태로 읽기 쉬움 | 🟡 XML이 장황함 |
| 빌드 속도 | 🟢 캐싱 + 증분 빌드로 빠름 | 🟡 상대적으로 느림 |
| Spring Initializr 기본값 | ✅ 기본 선택 | 옵션 |
| 기존 프로젝트 비율 | 신규 프로젝트에서 증가 | 레거시 프로젝트에서 많음 |

> **한 줄 요약**: 빌드 속도가 빠르고, DSL 기반이라 XML보다 읽기 쉬우며, Spring Initializr의 기본 선택이다.

#### 6) 왜 React인가? (vs Vue, Angular)

| 비교 항목 | React | Vue | Angular |
|-----------|-------|-----|---------|
| 설계 철학 | 라이브러리 (자유도 높음) | 프레임워크 (적절한 가이드) | 풀 프레임워크 (엄격한 구조) |
| 학습 곡선 | 중간 (JSX 적응 필요) | 🟢 낮음 (템플릿 기반) | 높음 (RxJS, DI 등) |
| TypeScript 지원 | 🟢 우수 | 🟢 우수 (Vue 3) | ✅ 기본 내장 |
| 커뮤니티/생태계 | 🟢 가장 큼 | 🟡 성장 중 | 🟡 엔터프라이즈 중심 |
| 채용 수요 | 🟢 가장 높음 | 🟡 국내 증가 중 | 🟡 대기업 일부 |

> **한 줄 요약**: 가장 큰 커뮤니티와 생태계를 가졌고, 컴포넌트 기반으로 재사용 가능한 UI를 만들기 좋다.
>
> **Vue와의 차이**: Vue는 학습 곡선이 낮아 빠르게 시작할 수 있지만, React은 생태계 규모와 취업 시장에서 앞선다.

#### 7) 왜 Redis인가? (vs Memcached, 캐시 없음)

| 비교 항목 | Redis | Memcached | 캐시 없음 |
|-----------|-------|-----------|----------|
| 데이터 구조 | Key-Value + 리스트, 셋, 해시 등 | Key-Value만 | - |
| TTL (자동 만료) | ✅ | ✅ | - |
| Pub/Sub | ✅ 지원 → 실시간 기능 활용 | ❌ | - |
| 영속성 | 선택적 디스크 저장 | ❌ 메모리만 | - |
| 활용 예 | 캐시, 세션, JWT Refresh Token, 실시간 알림 | 단순 캐시 | 매번 DB 조회 |

> **한 줄 요약**: 단순한 캐시를 넘어 TTL, Pub/Sub 등 다양한 기능을 제공해 JWT 토큰 관리와 실시간 기능에 활용할 수 있다.

#### 8) 왜 Docker인가?

| 비교 항목 | Docker 사용 | 직접 설치 |
|-----------|------------|----------|
| 환경 일관성 | ✅ 모든 환경 동일 | ❌ "내 컴퓨터에서는 되는데" 문제 |
| 설치/제거 | `docker-compose up/down` | 수동 설치/설정/제거 |
| 버전 관리 | 이미지 태그로 간편 | 직접 업그레이드 |
| 팀 협업 | `docker-compose.yml` 공유 한 장 | 각자 설치 가이드 필요 |

> **한 줄 요약**: "내 컴퓨터에서는 되는데" 문제를 해결하고, 설치 없이 DB를 사용할 수 있으며, 팀 전체가 동일한 환경을 공유한다.
